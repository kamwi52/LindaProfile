  ...
  const schoolData = {
    ...
    classrooms: {
      ...
      statistics: [
        ...
        { label: 'Student Capacity', value: '1800' },
    ...
    shn: {
      ...
      overview: 'Our comprehensive School Feeding Program ensures that learners have access to balanced nutrition, noting success in improved attendance.',
      benefits: [
        { icon: '🥗', title: 'Balanced Nutrition', description: 'Nutritionally balanced meals meeting dietary requirements for growing learners' },
        { icon: '🍛', title: '1400+ Meals Served', description: 'We serve over 1400+ balanced meals a day to our students' },
        { icon: '📈', title: 'Improved Attendance', description: 'Success: Noted improvement in learner attendance since program inception' },
        { icon: '💪', title: 'Health Screenings', description: 'Regular health checks and vaccinations for all students' },
      ],
      statistics: [
        { label: 'Daily Meals Served', value: '1400+' },
    ...
    <div className="min-h-screen bg-gradient-to-br from-white via-primary-50 to-white text-gray-900">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        {activeSection === 'classof' && (
          <section style={{ minHeight: 'calc(100vh - 70px)' }} className="bg-gradient-to-b from-white to-primary-50 p-6 md:p-12 flex flex-col justify-start pt-8">
            <h2 className="text-3xl md:text-4xl font-bold text-primary-500 mb-4">Past History</h2>
            <p className="text-primary-700 mb-8">Explore the rich history of Linda Secondary School and the remarkable journeys of our former students.</p>

            <div className="grid md:grid-cols-4 gap-8 flex-1">
              {/* Alumni Index Sidebar */}
              <div className="md:col-span-1 bg-white border-2 border-primary-100 rounded-xl overflow-hidden shadow-sm">
                <div className="bg-primary-500 p-3 text-white font-bold text-sm text-center tracking-wider uppercase">Graduates Index</div>
                <div className="overflow-y-auto max-h-[600px] divide-y divide-gray-100">
                  {alumniProfiles.map(alumnus => (
                    <button
                      ...
                      className={`w-full text-left p-4 transition-all hover:bg-primary-50 group ${selectedAlumnus?.id === alumnus.id ? 'bg-primary-50 border-l-4 border-primary-500' : ''}`}
                    >
                      <p className={`font-bold text-sm ${selectedAlumnus?.id === alumnus.id ? 'text-primary-600' : 'text-gray-700 group-hover:text-primary-500'}`}>{alumnus.name}</p>
              ...
              <div className="md:col-span-3">
                {selectedAlumnus ? (
                  <div className="bg-white border-2 border-primary-500 rounded-xl p-8 shadow-lg h-full overflow-y-auto">
                    <div className="flex flex-col md:flex-row md:items-start gap-8">
                      {/* Historical Images */}
                      <div className="flex-shrink-0 w-full md:w-1/3">
                        <div 
                          className="aspect-square bg-gray-100 rounded-xl overflow-hidden border-2 border-primary-100 hover:border-primary-500 transition-all cursor-pointer group relative shadow-md"
                          onClick={() => openImageSlideshow(selectedAlumnus.photos, 0, `${selectedAlumnus.name} History`)}
                        >
                          <img src={selectedAlumnus.photos[0]?.url || '/images/alumini/default.jpg'} className="w-full h-full object-cover" alt={selectedAlumnus.name} />
                          <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                            <ImageIcon className="w-12 h-12 text-white" />
                          </div>
                        </div>
                        <div className="grid grid-cols-2 gap-2 mt-4">
                          {selectedAlumnus.photos.slice(1).map((photo, index) => (
                            <div 
                              key={photo.id}
                              className="aspect-video bg-gray-100 rounded-lg overflow-hidden border border-primary-100 hover:border-primary-500 transition-all cursor-pointer group relative shadow-sm"
                              onClick={() => openImageSlideshow(selectedAlumnus.photos, index + 1, `${selectedAlumnus.name} History`)}
                            >
                              <img src={photo.url} className="w-full h-full object-cover" alt={`${selectedAlumnus.name} ${index + 2}`} />
                            </div>
                          ))}
                        </div>
                      </div>
                      <div className="flex-1">
                        <h3 className="text-2xl font-bold text-primary-500 mb-2">{selectedAlumnus.name} — Class of {selectedAlumnus.graduationYear}</h3>
                        <p className="text-primary-700 font-bold mb-4">{selectedAlumnus.class} • {selectedAlumnus.currentPosition}</p>
                        <div className="prose prose-gray max-w-none text-gray-700 space-y-4 leading-relaxed">
                          {selectedAlumnus.fullStory.split('\n\n').map((paragraph, idx) => (
                  ...
                  <div className="h-full flex items-center justify-center text-gray-400 italic bg-white border-2 border-dashed border-gray-200 rounded-xl">Select a graduate to view their story</div>
                )}
              </div>
            </div>
          </section>
